...
export function SdmInteractionLayer({
  ...
  const beginMarqueeRef = useRef<
    (event: PointerEvent, stage: HTMLElement) => void
  >(() => {});
  beginMarqueeRef.current = (event, stage) =>
    beginMarquee({
      event,
      ...
      track: trackGesture,
      preview: setMarquee,
      select: (ids) => onSelectRef.current(ids),
    });
    ...
    trackGesture<DragGestureValue>({
      ...
      commitValue: (value) => {
        ...
        if (nextDocument !== currentDocument) {
          onCommitRef.current(nextDocument, elementIds, currentDocument);
        }
      },
      restore: () => {
        const boundsNode = overlayRef.current?.querySelector(
  ...
  const beginRotation = (
    ...
    trackGesture({
      ...
      commitValue: (rotationDeg) => {
        ...
        if (nextDocument !== currentDocument) {
          onCommitRef.current(nextDocument, [element.id], currentDocument);
        }
      },
      restore: () => {
        const currentElement = documentRef.current.elements.find(
  ...
  return (
    ...
    >
      ...
      {multipleBounds !== null && marquee === null ? (
        ...
        >
          {multipleResizable ? HANDLES.filter(
            ...
            <div
              ...
              onPointerDown={(event) => {
                ...
                beginSelectionResize({
                  ...
                  tolerance: snapTolerance,
                  track: trackGesture,
                  preview: (next) => onPreviewRef.current(next),
                  commit: (next, ids, baseDocument) =>
                    onCommitRef.current(next, ids, baseDocument),
